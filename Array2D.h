...
public:
	...
	T Select(int row, int column); //to select an element of the Array2D object
...
private:
	...
	void m_dataAllocation(T **&new_array, int rows, int cols); //to allocate data for new arrays
	void m_dataTransfer(T **&new_array, int rows, int cols, bool conserve); //to conserve and transfer data between 2D arrays
};
